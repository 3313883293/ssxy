...
const GLOSSARY = {
    ...
    '燃烧': { title: '燃烧', desc: '持续伤害状态：回合结束时受到等级 × 50 真实伤害。每回合至少消耗 1 层（等级每满 5 级再多消耗 1 层）；层数耗尽时照样按当前等级正常结算一次，然后燃烧结束。' },
    ...
    '被制服': { title: '被制服', desc: '多能战警施加的压制状态：速度 -2（总共，无论层数）；回合结束层数 -1。' },
    '创伤': { title: '创伤', desc: '多能战警施加的伤口状态：拥有者自己投掷硬币时（出招投币之前）伤口崩裂——每次消耗 1 层并造成 级数×20 真实伤害，触发次数 = 本次投出的硬币总数（硬币越多越痛；不投币的技能不触发）。层数耗尽后消失；若崩裂致死，本次行动作废。' },
    '休整': { title: '休整', desc: '多能战警战术撤退补位入场后获得的状态：接下来的 3 个回合开始各回复 20% 血量；休整期间不会触发战术撤退（v0.687）。' }
};
...
function skillEffectLines(skill) {
    ...
    if (skill.special) {
        ...
        else if (s.type === 'subdue') lines.push(`【命中时】对目标施加 ${s.stacks} 层「被制服」与 2 层 Lv2「混乱」`);   // v0.684 近身制服
        else if (s.type === 'rifleBurn') lines.push(`【命中时】对目标施加 1 层 Lv2「燃烧」`);   // v0.684 中距点射
        else if (s.type === 'snipeTrauma') lines.push(`【命中时】对目标施加 ${s.stacks} 层 Lv2「创伤」`);   // v0.684 远程狙击
        else if (s.type === 'crossfire') lines.push(`【连携技】与至多 2 名多能战警连携（每名消耗 200 算力），对射程内全体敌方造成伤害；防爆装参与：对被制服目标 +200 伤害并对全体施加 3 层 Lv3「混乱」；步枪装参与：基础伤害 +200 并对全体施加 2 层 Lv2「燃烧」；狙击装参与：加成伤害 +400 并对全体施加 3 层 Lv3「创伤」`);   // v0.684 交叉火力
    }
    return lines;
}
