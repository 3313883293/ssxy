...
const GLOSSARY = {
    ...
    '恶': { title: '恶', desc: '每层使鲁盼旋对目标无视 50 防御；【十二连·剑斩邪祟】命中时可转化为每层 100 点额外伤害并清零。' },
    '愤怒': { title: '愤怒', desc: '鲁盼旋的专属强化状态（上限 5 级）：每 2 级基础伤害 +100、每 2 级防御 −50，不回复算力。回合结束时获得场上「恶」总层数的愤怒；友方阵亡时愤怒 +3 级；可消耗 1 级为「燃烧」追加 1 层。' },
    '怨恨': { title: '怨恨', desc: '云长郡的专属强化状态（上限 10 级）：受击减伤 = 100% − 等级×15%（Lv 7 起跌破 0%，转为受到伤害加成）；每 3 级基础伤害 +50、防御 −50（Lv 3/6/9 = +50/100/150）；同阵营（含其召唤的警察怨灵）阵亡时怨恨 +1 级；不回复算力。' },
    '情感激荡': { title: '情感激荡', desc: '情绪累积系统（0~8 级）：受击/攻击/击杀/队友死亡各 +1 级；达 2 级基础伤害 +50、达 6 级 +100、达 4 级算力回复 +50、达 8 级 +100（覆盖式）。' },
    '暂时昏迷': { title: '暂时昏迷', desc: '目标本回合无法行动，回合结束后恢复。' },
    '昏迷': { title: '暂时昏迷', desc: '目标本回合无法行动，回合结束后恢复。' },
    '狂炎': { title: '狂炎', desc: '焚天祭司·烛央的强化状态：每层使【烈焰鞭】/【焚天祭】伤害+150、防御-20。烛央【薪火不息】回合结束时把场上全体「燃烧」等级之和转为狂炎层数；死亡时【焚尽薪火】把狂炎转成残余敌方的「燃烧」等级。' },
    '易燃': { title: '易燃', desc: '焦木傀儡的固有特性：受到的「燃烧」持续伤害×1.5。' },
    '混乱': { title: '混乱', desc: '受击反噬型持续伤害：受到伤害后按本次攻击的分配硬币数触发反噬（次数=硬币数×0.5 向上取整），每次消耗 1 层并造成 级数×20 真实伤害。无回合末结算、无自然衰减，层数耗尽后混乱消失。' }
};
