...
// 补位入场后获得休整3回合）+ 创伤（v0.693 用户口径：**拥有者自己投币时**按本次投出的硬币总数崩裂自伤，
// 每次 级数×20 真伤并消耗 1 层；崩裂致死则本次出招作废）+ 被制服（速度-2、回合结束层数-1）
...
// ==================== 烬火教团（v0.5 灼华篇敌方；v0.692 数值规整为 50 的整数） ====================
// 主题：崇拜火焰的教团——焚香给队友挂燃烧、烛央薪火不息吸燃烧成狂炎而变强。
// 玩家用灼华烧教团既爽也喂火了 Boss：每层狂炎使烛央伤害+150、防御-20（双刃剑）。
// v0.692（用户口径变更）：本篇定位改为「燃烧角色的主场」——灼华是当章角色，应当能靠燃烧无脑打过；
//   三模板（模板一/二/三）也应当能过。**不再追求「非模板队无法无脑过」**，因此不引入任何反燃烧 / 反防护机制
//   （不做 v0.691 试过的「以火为食 / 焚身之焰」，狂炎也保持无层数上限）。本次只做两件事：
//   ① 数值微调：教团全部面板与技能数值就近取整为 50 的整数（量级与机制不变，逐项见下）；
//   ② 燃烧 dot 取消「每层只供给 5 级」的真伤上限（battleFlow.onTurnEnd 燃烧段），层数耗尽时照常按等级结算。
// v0.690 的难度上调（血量/防御/伤害/算力回复）保持原样，仅做 50 整数对齐：
//   ①烬火信徒 2600 血、防御 120→100、算力 340+130→350+150、火球术 230+220→250+200、火刃 300+260→300+250；
//   ②焦木傀儡 4500 血、防御 260→250、算力 340+110→350+100、重锤 500+320→500+300、木甲 250 算力/+200 防不变；
//   ③引火学徒 2300 血、防御 120→100、算力 380+150→400+150、燎原 200+190→200+200、引火 180+160→200+150；
//   ④焚香祭司 3600 血、防御 190→200、算力 520+170→500+150、祭火 400+340→400+350；
//   ⑤烛央 7600 血、防御 190→200、算力回复 420→400、烈焰鞭 470+470→450+450、焚天祭 190+190→200+200
//     （狂炎每层 +150 伤害 / −20 防御不变，无层数上限）。
// 同步点：charSelect.getRoleDefRange 的防御表须与本文件一致（防御有变动）。

function createAshCultist(team, position) {
    const skills = [
        new Skill('火球术', 250, 250, 200, 2, 3, null, { type: 'burn', stacks: 2 }),
        new Skill('火刃', 200, 300, 250, 1, 2)
    ];
    return new Character('烬火信徒', 2600, 100, [2,5], 350, 150, skills, team, position);
}

function createCharredGolem(team, position) {
    const skills = [
        new Skill('木甲', 250, 0, 0, 0, 1, { type: 'def', value: 200, duration: 'nextHit' }),
        new Skill('重锤', 300, 500, 300, 1, 2)
    ];
    const char = new Character('焦木傀儡', 4500, 250, [1,3], 350, 100, skills, team, position);
...
function createFirestarter(team, position) {
    const skills = [
        new Skill('燎原', 250, 200, 200, 2, 3),
        new Skill('引火', 200, 200, 150, 1, 4, null, { type: 'burn', stacks: 1 })
    ];
    return new Character('引火学徒', 2300, 100, [4,8], 400, 150, skills, team, position);
}

function createIncensePriest(team, position) {
    const skills = [
        new Skill('焚香', 250, 0, 0, 0, 99, null, { type: 'incense' }),
        new Skill('祭火', 400, 400, 350, 2, 4, null, { type: 'burn', stacks: 1 })
    ];
    const char = new Character('焚香祭司', 3600, 200, [2,4], 500, 150, skills, team, position);
...
function createZhuYang(team, position) {
    const skills = [
        new Skill('烈焰鞭', 300, 450, 450, 1, 3),
        new Skill('焚天祭', 500, 200, 200, 3, 4, null, { type: 'burn', stacks: 1 }),
        new Skill('火灵召唤', 350, 0, 0, 0, 99, null, { type: 'summon', role: '烬火信徒' })
    ];
    const char = new Character('焚天祭司·烛央', 7600, 200, [3,6], 800, 400, skills, team, position);
    ...
    // v0.692：沿用 v0.690 的无层数上限（用户口径：不做反制机制）——烧得越猛它越疯，玩家用灼华速攻即可；
    //   是否失控由 diag-squad.js 实测把关（模板队可过 + 灼华燃烧队无脑过、无僵局）。
    char.registerPassive('onTurnEnd', (self, bs, log) => {
