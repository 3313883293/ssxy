...
function getAvailableChars() {
    ...
    // v0.669：王庄明默认解锁（暂无解锁途径，用户指定进可选池）
    // v0.673：曹佳梦默认解锁（用户指定进可选池）
...
const AVAILABLE_ENEMIES = ['模板一', '模板二', '模板三', '鲁盼旋', '灼华', '张子曦', '王庄明', '曹佳梦', '纸糊稻草人', '铁皮稻草人', '标准稻草人', '灵敏稻草人', '再生稻草人', '持盾警察', '持棍警察', '持枪警察', '开车警察', '李雅礼', '云长郡', '多能战警', '烬火信徒', '焦木傀儡', '引火学徒', '焚香祭司', '焚天祭司·烛央'];   // v0.684 测试关敌人池加入多能战警
...
const PASSIVE_INFO = {
    '鲁盼旋': [
        ...
        '惩恶之火：友方受伤时，伤害来源获得 1 层「恶」；回合结束时，获得场上总「恶」层数的「愤怒」；队友死亡时愤怒 +3 级；技能命中时施加分配硬币数级「燃烧」（目标「燃烧」≤3 层且自身「愤怒」≥1 级时，消耗 1 级额外 +1 层）',
        '愤怒（上限 5 级）：每 2 级基础伤害 +100、每 2 级防御 -50；不回复算力（不受通用触发影响）'
    ],
    '模板一': [], '模板二': [], '模板三': [],
    '持盾警察': [], '持棍警察': [], '持枪警察': [],
    '开车警察': ['技能循环（AI）：【加油】×2 → 【开创】 → 【刹车】'],
    '李雅礼': ['倒戈：死亡后在我方队伍最右方（紧挨敌人）复活，仍由 AI 操控'],
    '云长郡': [
        '「怨恨」（特殊情感激荡，上限 10 级）：受击减伤 = 100% − 怨恨等级×15%，跌破 0% 转为受到伤害加成；每 3 级基础伤害 +50、防御 -50；队友死亡时怨恨 +1 级（含召唤的警察怨灵）；不回复算力；减伤每回合开始判定',
    ...
    // v0.5 烬火教团
    '烬火信徒': [],
    '焦木傀儡': ['易燃：燃烧结算时，受到的「燃烧」dot 伤害×1.5'],
    '引火学徒': [],
    ...
    '王庄明': [
        '黎明级先天能力者：回合结束时，若本回合未使用技能，则回复 200 算力',
        '守护之躯：回合结束时，本回合每消耗 100 算力获得 10% 减伤（向下取整、无上限），持续到下回合结束'
    ],
    // v0.673 曹佳梦（概率赌徒）
    '曹佳梦': [
        '黎明级书生学院校服：受到的直接伤害减免 20%',
        '概率论的奇迹：在场时，队友硬币投正率每个曹佳梦 +10%（可叠加），曹佳梦自身 +25% 且每级「厌倦」使自身投正率 -5%，并吃场上其他曹佳梦各 +10%',
        '厌倦（特殊情感激荡，上限 5 级）：每投出一个正面硬币 +1 级（【精准狙击】投正时额外 +1 级），每级基础伤害 +50；使用【陨星落下】后归零；厌倦 ≥4 级时回合开始【创大运吧】替换为【陨星落下】（<4 级恢复）；不回复算力'
    ],
    // v0.684 多能战警（张子曦篇第一关敌方）
    '多能战警': [
        '装备切换：回合开始根据位置切换装备——同阵营多能战警中位置最前 → 防爆装（防御+300，使用【近身制服】）；次前 → 步枪装（防御+100、速度+1，使用【中距点射】）；其余 → 狙击装（速度+3，使用【远程狙击】）；只能使用当前装备技能与【交叉火力】',   // v0.688 技能限制
        '战术撤退：回合结束时若自身血量不高于 40%，退回待命区末端（一局仅触发一次）；每回合开始待命区按序补位进场，重新上场时获得「休整」3 回合（回合开始回复 20% 血量，休整期间不会再次撤退）'   // v0.687 休整期不撤退 + v0.688 一局一次/每回合补位
    ]
};
